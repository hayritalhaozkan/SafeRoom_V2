...
public class P2PConnectionManager {
    ...
    public void initialize(String username) {
        ...
        try {
            com.saferoom.webrtc.CallManager callManager = com.saferoom.webrtc.CallManager.getInstance();
     ...
     * 
     * IMPORTANT: Now using MESH_* signal types for consistency with
     * GroupCallManager
    ...
    public CompletableFuture<Boolean> createConnection(String targetUsername) {
        ...
        try {
            ...
            // Store in activeConnections immediately so we can handle P2P_ANSWER and ICE
            // candidates
     ...
     * the registered handlers (handleMeshOffer, handleMeshAnswer,
        ...
        try {
            ...
            connection.peerConnection.setRemoteDescription(remoteDesc, new SetSessionDescriptionObserver() {
                ...
                public void onSuccess() {
                    ...
                    // Create answer
                    connection.peerConnection.createAnswer(new RTCAnswerOptions(),
                                    ...
                                    connection.peerConnection.setLocalDescription(description,
                                            new SetSessionDescriptionObserver() {
                                                ...
                                                public void onSuccess() {
                                                    ...
                                                    System.out.printf("[P2P] MESH_ANSWER sent to %s (roomId: %s)%n",
                                                            remoteUsername, incomingRoomId);
                                                }

                                                @Override
                                                public void onFailure(String error) {
                                                    System.err.printf("[P2P] Failed to set local description: %s%n",
                                                            error);
                                                }
                                            });
                                }
    ...
    /**
     * Send file via WebRTC DataChannel with file transfer protocol and observer
     * callbacks
     */
        ...
        try {
            ...
            return readyFuture.orTimeout(RECEIVER_READY_TIMEOUT_SEC, TimeUnit.SECONDS)
                    ...
                    .exceptionally(ex -> {
                        ...
                        if (observer != null)
                            observer.onTransferFailed(fileId, ex);
                        return false;
                    });
        } catch (Exception e) {
    ...
    private class P2PConnection {
        ...
        // File transfer: DataChannelFileTransfer coordinates roles, uses original
        // file_transfer/
        ...
        void createPeerConnection() {
            ...
            // Demo credentials from Metered's documentation - may have usage limits
            ...
            System.out.printf("[P2P] Configuring peer connection with %d ICE servers (STUN + TURN)%n",
                    iceServers.size());
            ...
            reliableMessaging.setCompletionCallback((sender, messageId, messageBytes) -> {
                try {
                    ...
                    javafx.application.Platform.runLater(() -> {
                        try {
                            com.saferoom.gui.service.ChatService chatService = com.saferoom.gui.service.ChatService
                                    .getInstance();
         ...
         * Uses DataChannelFileTransfer which coordinates roles and uses original
         * file_transfer/
         */
        ...
        void handleMessagingChannelMessage(RTCDataChannelBuffer buffer) {
            try {
                ...
                if (signal >= 0x20 && signal <= 0x23) {
                    ...
                    System.err.printf("[MSG-CH-RECV] WARNING: Unexpected signal on messaging channel: 0x%02X%n",
                            signal);
                }
        ...
        private void sendControlMessage(String payload) {
            ...
            // 🔥 CRITICAL FIX: Use DIRECT DataChannel for file transfer control (bypass
            // reliable messaging)
            ...
            try {
                ...
                // Use signal 0xF0 for file transfer control (different from messaging
                // 0x20-0x23)

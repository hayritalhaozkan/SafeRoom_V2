...
public class ContactService {
    ...
    public void loadFriendsAsContacts(String username) {
        ...
        javafx.concurrent.Task<java.util.List<com.saferoom.grpc.SafeRoomProto.FriendInfo>> task = new javafx.concurrent.Task<>() {
            @Override
            protected java.util.List<com.saferoom.grpc.SafeRoomProto.FriendInfo> call() throws Exception {
                com.saferoom.grpc.SafeRoomProto.FriendsListResponse response = com.saferoom.client.ClientMenu
        ...
        task.setOnSucceeded(event -> {
            ...
            javafx.application.Platform.runLater(() -> {
                ...
                // 🚀 AUTO-LOAD: Load only previews (Lazy Loading)
                loadLastMessagesForPreviews(friends);
            });
        });
    ...
    /**
     * 🚀 Load ONLY the last message for each conversation to populate the contact
     * list.
     * This avoids decrypting thousands of messages at startup.
     */
    private void loadLastMessagesForPreviews(java.util.List<com.saferoom.grpc.SafeRoomProto.FriendInfo> friends) {
        System.out.println("[ContactService] 🚀 Loading last message previews...");
        ...
        for (com.saferoom.grpc.SafeRoomProto.FriendInfo friend : friends) {
            ...
            // Load only the most recent message (Limit 1)
            // Use the convenience method on ChatService
            chatService.loadRecentMessages(friendUsername, 1)
                    .thenAccept(messages -> {
                        if (messages != null && !messages.isEmpty()) {
                            System.out.printf("[ContactService] 📂 Loaded preview for: %s%n", friendUsername);

                            // Update last message in contact list from loaded history
                            javafx.application.Platform.runLater(() -> {
                                updateLastMessageFromHistory(friendUsername, messages);
                            });
                        }
                    })
                    .exceptionally(error -> {
                        System.err.printf("[ContactService] ❌ Failed to load preview for %s: %s%n",
    ...
    private void updateLastMessageFromHistory(String contactId,
            java.util.List<com.saferoom.gui.model.Message> messages) {
        if (messages != null && !messages.isEmpty()) {
            // Get the first message (NEWEST) because SQL returns DESC
            com.saferoom.gui.model.Message lastMsg = messages.get(0);
